...
function renderLedPreview(fraction, blinking){ const preview = $('#ledPreview'); if(!preview){ return; } const dots = $$('.led-dot', preview); if(!dots.length){ return; } const mode = parseInt($('#modeSelect')?.value || '0', 10); const greenEnd = parseInt($('#greenEndSlider')?.value || '0', 10) / 100; const yellowEnd = parseInt($('#yellowEndSlider')?.value || '0', 10) / 100; const redEnd = parseInt($('#redEndSlider')?.value || '0', 10) / 100; const blinkStart = parseInt($('#blinkStartSlider')?.value || '0', 10) / 100; const blinkTrigger = Math.max(redEnd, blinkStart); const startRpm = parseInt($('#rpmStartSlider')?.value || '0', 10); const previewMaxRpm = 8000; const startFraction = Math.min(0.95, Math.max(0, startRpm / previewMaxRpm)); const colors = { green: $('#greenColorInput')?.value || '#2DFF7A', yellow: $('#yellowColorInput')?.value || '#FFC34D', red: $('#redColorInput')?.value || '#FF5A72', pit: '#ff3fd2' }; const blinkOn = Math.floor(Date.now() / 160) % 2 === 0; const rawFrac = Math.max(0, Math.min(1, fraction)); const frac = rawFrac <= startFraction ? 0 : Math.max(0, Math.min(1, (rawFrac - startFraction) / Math.max(0.01, 1 - startFraction))); const dark = '#26384d'; if(mode === 3){ const pairCount = Math.ceil(dots.length / 2); const pairsOn = Math.round(frac * pairCount); const finalBlink = blinking && frac >= blinkTrigger; dots.forEach((dot, index) => { let color = dark; if(finalBlink){ color = blinkOn ? colors.red : '#091019'; } else { const rank = Math.min(index, dots.length - 1 - index); if(rank < pairsOn){ const pos = pairCount <= 1 ? 1 : rank / (pairCount - 1); color = pos < greenEnd ? colors.green : (pos < yellowEnd ? colors.yellow : colors.red); } } dot.style.backgroundColor = color; }); if(frac <= 0){ [0,1,dots.length-2,dots.length-1].forEach((idx) => { if(dots[idx]){ dots[idx].style.backgroundColor = dark; } }); } return; } const onCount = Math.round(frac * dots.length); dots.forEach((dot, index) => { let color = dark; if(index < onCount){ const pos = dots.length <= 1 ? 1 : index / (dots.length - 1); if(mode === 2 && frac >= blinkTrigger){ color = blinkOn ? colors.red : '#091019'; } else if(pos < greenEnd){ color = colors.green; } else if(pos < yellowEnd){ color = colors.yellow; } else if(blinking && mode === 1 && frac >= blinkTrigger){ color = blinkOn ? colors.red : '#091019'; } else { color = colors.red; } } dot.style.backgroundColor = color; }); }
...
function updateStatus(data){
  ...
  updateText('ledDebugValue', `show ${data.ledFrameShows ?? 0} | skip ${data.ledFrameSkips ?? 0} | mode ${(data.ledRenderMode || '-')} | writer ${(data.ledLastWriter || '-')}`);
