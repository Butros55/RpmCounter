...
namespace
{
    ...
    void appendShellHead(String &page, const __FlashStringHelper *title, bool settingsActive)
    {
        ...
        page += F("</title><style>");
        page += F(R"CSS(
:root{--bg:#08111b;--panel:#101a27;--panel-2:#121f2f;--text:#eef5ff;--muted:#8da2ba;--accent:#4bb7ff;--success:#40d39c;--warn:#ffb84d;--danger:#ff6f81;--border:#24364a;--border-strong:#33506d;--radius:18px;--radius-sm:12px}
...
.app{width:min(1180px,100%);margin:0 auto;padding:calc(18px + env(safe-area-inset-top)) 16px calc(116px + env(safe-area-inset-bottom))}
...
.app-grid,.panel-grid,.field-grid,.stack,.info-list{display:grid;gap:16px}.panel{border:1px solid var(--border);background:linear-gradient(180deg,rgba(16,26,39,.95),rgba(12,20,31,.95));border-radius:var(--radius);padding:18px}
...
const dashboardState = { dirty:false, pending:0, initial:null, previewTimer:null, testSweepActive:false, testSweepStart:0, blinkPreview:false, blinkPreviewUntil:0 };
...
function enforceOrder(changedId){ const green = $('#greenEndSlider'); const yellow = $('#yellowEndSlider'); const blink = $('#blinkStartSlider'); let g = parseInt(green.value || '0', 10); let y = parseInt(yellow.value || '0', 10); let b = parseInt(blink.value || '0', 10); if(changedId === 'greenEndSlider' && y < g){ y = g; } if(changedId !== 'blinkStartSlider' && b < y){ b = y; } if(changedId === 'yellowEndSlider' && b < y){ b = y; } green.value = String(g); yellow.value = String(y); blink.value = String(b); [green,yellow,blink].forEach(updateSliderValue); }
...
function renderLedPreview(fraction, blinking){ const preview = $('#ledPreview'); if(!preview){ return; } const dots = $$('.led-dot', preview); if(!dots.length){ return; } const mode = parseInt($('#modeSelect')?.value || '0', 10); const greenEnd = parseInt($('#greenEndSlider')?.value || '0', 10) / 100; const yellowEnd = parseInt($('#yellowEndSlider')?.value || '0', 10) / 100; const blinkStart = parseInt($('#blinkStartSlider')?.value || '0', 10) / 100; const colors = { green: $('#greenColorInput')?.value || '#2DFF7A', yellow: $('#yellowColorInput')?.value || '#FFC34D', red: $('#redColorInput')?.value || '#FF5A72', pit: '#ff3fd2' }; const blinkOn = Math.floor(Date.now() / 160) % 2 === 0; const frac = Math.max(0, Math.min(1, fraction)); const dark = '#26384d'; if(mode === 3){ const pairCount = Math.ceil(dots.length / 2); const pairsOn = Math.round(frac * pairCount); const finalBlink = blinking && frac >= blinkStart; dots.forEach((dot, index) => { let color = dark; if(finalBlink){ color = blinkOn ? colors.red : '#091019'; } else { const rank = Math.min(index, dots.length - 1 - index); if(rank < pairsOn){ const pos = pairCount <= 1 ? 1 : rank / (pairCount - 1); color = pos < greenEnd ? colors.green : (pos < yellowEnd ? colors.yellow : colors.red); } } dot.style.backgroundColor = color; }); if(!finalBlink && frac < 0.08){ [0,1,dots.length-2,dots.length-1].forEach((idx) => { if(dots[idx]){ dots[idx].style.backgroundColor = colors.pit; } }); } return; } const onCount = Math.round(frac * dots.length); dots.forEach((dot, index) => { let color = dark; if(index < onCount){ const pos = dots.length <= 1 ? 1 : index / (dots.length - 1); if(mode === 2 && frac >= blinkStart){ color = blinkOn ? colors.red : '#091019'; } else if(pos < greenEnd){ color = colors.green; } else if(pos < yellowEnd){ color = colors.yellow; } else if(blinking && mode !== 0 && frac >= blinkStart){ color = blinkOn ? colors.red : '#091019'; } else { color = colors.red; } } dot.style.backgroundColor = color; }); }
function updatePreviewLoop(){ if(dashboardState.testSweepActive){ const elapsed = Date.now() - dashboardState.testSweepStart; const t = Math.min(1, elapsed / TEST_SWEEP_DURATION); renderLedPreview(ledFractionAt(t), true); if(t >= 1){ dashboardState.testSweepActive = false; } return; } if(dashboardState.blinkPreview){ renderLedPreview(1, true); if(Date.now() >= dashboardState.blinkPreviewUntil){ dashboardState.blinkPreview = false; } return; } renderLedPreview(1, false); }
...
function updateStatus(data){ if(!data){ return; } updateText('heroRpm', data.rpm ?? 0); updateText('heroMaxRpm', data.maxRpm ?? 0); updateText('heroSpeed', (data.speed ?? 0) + ' km/h'); updateText('heroGear', data.gear ?? 0); updateText('debugLastTx', data.lastTx || 'Noch kein TX'); updateText('debugLastObd', data.lastObd || 'Noch keine Antwort'); updateText('vehicleModelValue', data.vehicleModel || 'Noch kein Modell'); updateText('vehicleVinValue', data.vehicleVin || 'Noch keine VIN'); updateText('vehicleDiagValue', data.vehicleDiag || 'Keine Diagnose'); updateText('bleSummaryValue', data.usbBridgeConnected ? (data.usbHost || 'USB Bridge') : (data.bleText || 'Getrennt')); const usbActive = data.activeTelemetry === 'USB Sim' || data.simTransport === 'USB' || data.wifiSuspended; setPill('telemetryPill', usbActive ? 'ok' : (data.activeTelemetry === 'SimHub' ? 'neutral' : (data.connected ? 'ok' : 'warn')), data.activeTelemetry || 'Keine'); setPill('simHubPill', usbActive ? (data.usbState === 'USB live' ? 'ok' : 'warn') : (data.simHubState === 'Live' ? 'ok' : (data.simHubConfigured ? 'warn' : 'bad')), usbActive ? (data.usbState || 'USB') : (data.simHubState || 'Keine SimHub-Daten')); setPill('blePill', usbActive ? (data.obdAllowed ? 'warn' : 'neutral') : (data.connected ? 'ok' : (data.bleConnectInProgress ? 'warn' : 'bad')), usbActive ? 'OBD deaktiviert' : (data.connected ? 'OBD verbunden' : (data.bleConnectInProgress ? 'OBD verbindet' : 'OBD offline'))); const connectBtn = $('#btnConnect'); const disconnectBtn = $('#btnDisconnect'); if(connectBtn && disconnectBtn){ if(data.obdAllowed === false){ connectBtn.classList.add('hidden'); disconnectBtn.classList.add('hidden'); } else { connectBtn.classList.toggle('hidden', !!data.connected); disconnectBtn.classList.toggle('hidden', !data.connected); } } }
...
function startSweep(){ const form = $('#mainForm'); if(!form){ return; } const done = beginRequest(); setButtonLoading('btnTest', true, 'Sweep laeuft'); fetch('/test', { method:'POST', body:new FormData(form) }).finally(() => { setButtonLoading('btnTest', false); done(); }); dashboardState.testSweepActive = true; dashboardState.testSweepStart = Date.now(); dashboardState.blinkPreview = false; ensurePreviewLoop(); }
function initLedPreview(){ const preview = $('#ledPreview'); if(!preview){ return; } preview.innerHTML = ''; for(let i=0;i<NUM_LEDS;i += 1){ const dot = document.createElement('span'); dot.className = 'led-dot'; preview.appendChild(dot); } renderLedPreview(1, false); ensurePreviewLoop(); }
function initDashboard(){ captureInitialState(); recomputeDirty(); initLedPreview(); syncAutoscaleUi(); updatePreviewLoop(); $$('input,select,textarea', $('#mainForm')).forEach((el) => { if(el.type === 'range'){ updateSliderValue(el); el.addEventListener('input', () => { if(el.id === 'greenEndSlider' || el.id === 'yellowEndSlider' || el.id === 'blinkStartSlider'){ enforceOrder(el.id); if(el.id === 'blinkStartSlider'){ triggerBlinkPreview(); } updatePreviewLoop(); } else { updateSliderValue(el); } markDirty(); }); el.addEventListener('change', markDirty); } else { el.addEventListener('input', markDirty); el.addEventListener('change', markDirty); } }); $('#autoscaleToggle')?.addEventListener('change', () => { syncAutoscaleUi(); markDirty(); }); $('#modeSelect')?.addEventListener('change', () => { updatePreviewLoop(); markDirty(); }); $('#brightnessSlider')?.addEventListener('input', (ev) => onBrightnessInput(ev.target.value)); $('#brightnessSlider')?.addEventListener('change', (ev) => onBrightnessInput(ev.target.value)); ['greenColorInput','yellowColorInput','redColorInput'].forEach((id) => { document.getElementById(id)?.addEventListener('input', () => { updatePreviewLoop(); markDirty(); }); }); $('#btnSave')?.addEventListener('click', () => saveForm('/save')); $('#btnReset')?.addEventListener('click', () => window.location.reload()); $('#btnTest')?.addEventListener('click', startSweep); $('#btnConnect')?.addEventListener('click', () => postSimple('/connect').then(fetchStatus)); $('#btnDisconnect')?.addEventListener('click', () => postSimple('/disconnect').then(fetchStatus)); $('#btnDisplayStatus')?.addEventListener('click', refreshDisplayStatus); $('#btnDisplayBars')?.addEventListener('click', () => { const done = beginRequest(); fetch('/dev/display-pattern', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'pattern=bars' }).finally(() => { done(); refreshDisplayStatus(); }); }); $('#btnDisplayGrid')?.addEventListener('click', () => { const done = beginRequest(); fetch('/dev/display-pattern', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'pattern=grid' }).finally(() => { done(); refreshDisplayStatus(); }); }); $('#btnDisplayLogo')?.addEventListener('click', () => postSimple('/dev/display-logo').then(refreshDisplayStatus)); fetchStatus(); setInterval(fetchStatus, 2200); if($('#displayStatusPill')){ refreshDisplayStatus(); } }
...
String buildDashboardPage()
{
    ...
    page += "<div class='metric'><div class='metric-label'>Gang</div><div class='metric-value metric-value--compact' id='heroGear'>" + String(g_estimatedGear) + "</div><div class='metric-note'>" + htmlEscape(currentIpString()) + "</div></div></div></div>";
    ...
    page += F("<form id='mainForm' action='/save' method='POST'><div class='app-grid'><section class='panel stack'><div class='panel-head'><div><h2 class='panel-title'>Allgemein</h2><div class='panel-copy'>Die haeufigsten Einstellungen bleiben oben und klar erreichbar.</div></div></div><div class='field-grid two'><div class='field'><label for='modeSelect'>Schaltmodus</label><select id='modeSelect' name='mode'>");
    page += "<option value='0'" + selectedAttr(cfg.mode == 0) + ">Casual</option><option value='1'" + selectedAttr(cfg.mode == 1) + ">F1-Style</option><option value='2'" + selectedAttr(cfg.mode == 2) + ">Aggressiv</option><option value='3'" + selectedAttr(cfg.mode == 3) + ">GT3 / Endurance</option></select><div class='field-note'>GT3 laeuft von aussen nach innen, blinkt am Ende komplett rot und zeigt bei Pit-Limiter magenta Marker aussen.</div></div>";
    page += "<div class='field'><label for='brightnessSlider'>LED-Helligkeit</label><input type='range' id='brightnessSlider' min='0' max='255' value='" + String(cfg.brightness) + "'><div class='field-note'>Aktuell <span id='brightnessValue'>" + String(cfg.brightness) + "</span> / 255</div><input type='hidden' id='brightnessHidden' name='brightness' value='" + String(cfg.brightness) + "'></div></div><div class='button-row'><button type='button' class='btn btn-secondary' id='btnTest'>RPM-Sweep testen</button>";
    ...
    page += "<div class='info-row'><div class='info-label'>BLE Status</div><div class='info-value' id='bleSummaryValue'>" + htmlEscape(bleSummary()) + "</div></div></div></section></div>";
    ...
    page += "<div class='range-wrap'><div class='range-head'><div class='range-title'>" + htmlEscape(redLabel) + "</div><div class='range-value' id='blinkStartValue'>" + String(cfg.blinkStartPct) + "%</div></div><input type='range' name='blinkStartPct' id='blinkStartSlider' min='0' max='100' value='" + String(cfg.blinkStartPct) + "' data-value-target='blinkStartValue' data-suffix='%'><div class='seg-note'>Ab hier beginnt die Shift-/Blink-Phase.</div></div><div><div class='field-label'>LED Vorschau</div><div id='ledPreview' class='led-preview'></div></div></section>";
    page += F("<div class='panel-grid two'><section class='panel stack'><div class='panel-head'><div><h2 class='panel-title'>Farbzonen</h2><div class='panel-copy'>Direkt, klar und ohne ueberfluessige UI-Last.</div></div></div><div class='color-grid'>");
    ...
    page += F("><span class='slider'></span></label></div></section></div>");
    ...
    page += "<div class='info-row'><div class='info-label'>Letzte OBD Zeile</div><div class='info-value mono' id='debugLastObd'>" + htmlEscape(g_lastObdInfo.isEmpty() ? String(F("Noch keine Antwort")) : g_lastObdInfo) + "</div></div></div><div class='button-row'><button type='button' class='btn btn-secondary' id='btnDisplayStatus'>Status aktualisieren</button><button type='button' class='btn btn-secondary' id='btnDisplayBars'>Farb-Balken</button><button type='button' class='btn btn-secondary' id='btnDisplayGrid'>Raster</button><button type='button' class='btn btn-secondary' id='btnDisplayLogo'>Logo anzeigen</button></div></div></details></form>";
...
const $ = (selector, scope=document) => scope.querySelector(selector);
const $$ = (selector, scope=document) => Array.from(scope.querySelectorAll(selector));
const settingsState = { dirty:false, pending:0, initial:null, refreshActive:false, wifiModalSsid:'' };
...
function saveSettings(){ const form = $('#settingsForm'); if(!form){ return; } const fd = new FormData(form); const params = new URLSearchParams(); fd.forEach((value, key) => params.append(key, value)); const done = beginRequest(); setButtonLoading('settingsSave', true, 'Speichere'); fetch('/settings', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:params.toString() }).then(() => { captureInitialSettingsState(); recomputeSettingsDirty(); $('#savedToast')?.classList.remove('hidden'); }).finally(() => { setButtonLoading('settingsSave', false); done(); }); }
function setVehicleStatus(text, error){ updateText('vehicleStatus', text); updateText('settingsError', error || ''); }
function updateStatus(data){ if(!data){ return; } const usbActive = data.activeTelemetry === 'USB Sim' || data.simTransport === 'USB' || data.wifiSuspended; setPill('settingsTelemetryPill', usbActive ? 'ok' : (data.activeTelemetry === 'SimHub' ? 'neutral' : (data.connected ? 'ok' : 'warn')), data.activeTelemetry || 'Keine'); setPill('settingsSimHubPill', usbActive ? (data.usbState === 'USB live' ? 'ok' : 'warn') : (data.simHubState === 'Live' ? 'ok' : (data.simHubConfigured ? 'warn' : 'bad')), usbActive ? (data.usbState || 'USB') : (data.simHubState || 'Deaktiviert')); updateText('activeTelemetryValue', data.activeTelemetry || 'Keine'); updateText('simHubStateValue', usbActive ? (data.usbState || 'USB') : (data.simHubState || 'Deaktiviert')); updateText('vehicleModel', data.vehicleModel || 'Noch kein Modell'); updateText('vehicleVin', data.vehicleVin || 'Noch keine VIN'); updateText('vehicleDiag', data.vehicleDiag || 'Keine Diagnose'); if(usbActive){ updateText('wifiModeValue', 'USB Bridge'); updateText('wifiIpValue', data.usbHost || 'USB lokal'); } if(data.vehicleInfoRequestRunning){ setVehicleStatus('Abruf laeuft', ''); } else if(data.vehicleInfoReady){ setVehicleStatus(data.vehicleInfoAge <= 1 ? 'Gerade aktualisiert' : ('Letztes Update vor ' + data.vehicleInfoAge + 's'), ''); } else { setVehicleStatus('Noch keine Daten', ''); } if(settingsState.refreshActive && !data.vehicleInfoRequestRunning){ settingsState.refreshActive = false; if(data.vehicleInfoReady){ setVehicleStatus('Gerade aktualisiert', ''); } else { setVehicleStatus('Sync nicht moeglich', 'Keine Antwort vom Fahrzeug.'); } } }
...
function updateWifiUi(data){ if(!data){ return; } const summary = data.staConnected ? (data.currentSsid || 'Verbunden') : (data.staConnecting ? 'Verbindung laeuft' : 'Offline'); updateText('wifiSummaryValue', summary); updateText('wifiModeValue', data.mode || 'AP_ONLY'); updateText('wifiIpValue', data.ip || data.apIp || '-'); setPill('settingsWifiPill', data.staConnected ? 'ok' : (data.apActive || data.staConnecting ? 'warn' : 'bad'), data.staConnected ? 'WLAN online' : (data.apActive ? 'AP aktiv' : 'WLAN offline')); updateText('wifiScanStatus', data.scanRunning ? 'Netzwerksuche laeuft' : (data.staConnecting ? 'Verbindung wird aufgebaut' : (data.staLastError || 'Bereit'))); if(data.currentSsid && !$('#staSsid').value){ $('#staSsid').value = data.currentSsid; } renderWifiResults(data); }
...
String buildSettingsPage(bool savedNotice)
{
    ...
    page += F("<div class='hero-card'><div class='hero-head'><div><div class='hero-kicker'>Hinweis</div><div class='hero-title'>Sim / PC Modus</div><div class='hero-sub'>Netzwerk-SimHub bleibt moeglich. Fuer den neuen USB-Sim-Mode dient eine lokale PC-Bridge als USB-Transport und lokale Weboberflaeche.</div></div></div><div class='callout'>In <strong>USB Serial Bridge</strong> pausiert der ESP32 BLE / OBD und WLAN automatisch, sobald die PC-Bridge aktiv ist. Im Netzwerkmodus liest der ESP32 SimHub weiter direkt ueber Host + Port.</div></div></section>");
    ...
    page += "<option value='1'" + selectedAttr(cfg.simTransportPreference == SimTransportPreference::UsbSerial) + ">USB Serial Bridge</option>";
    page += "<option value='2'" + selectedAttr(cfg.simTransportPreference == SimTransportPreference::Network) + ">Netzwerk / SimHub API</option>";
    page += F("</select><div class='field-note'>USB Sim schaltet BLE und WLAN auf dem ESP ab, sobald die lokale PC-Bridge verbunden ist.</div></div>");
