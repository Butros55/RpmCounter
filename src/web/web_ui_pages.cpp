...
const dashboardState = { dirty:false, pending:0, initial:null, previewTimer:null, testSweepActive:false, testSweepMode:'show', testSweepStart:0, blinkPreview:false, blinkPreviewUntil:0, lastAmbientAutoProbeMs:0, lastGestureAutoProbeMs:0 };
...
function noStoreUrl(url){ const joiner = url.includes('?') ? '&' : '?'; return url + joiner + '_ts=' + Date.now(); }
function fetchJsonNoStore(url, init){ const opts = Object.assign({ cache:'no-store' }, init || {}); return fetch(noStoreUrl(url), opts); }
...
function syncSelectValue(id, value, syncInitial=false){ const el = document.getElementById(id); if(!el || value === undefined || value === null){ return false; } const next = String(value); const changed = el.value !== next; if(changed){ el.value = next; } if(syncInitial && dashboardState.initial && el.name){ dashboardState.initial[el.name] = next; } return changed; }
...
function refreshDisplayStatus(){ const done = beginRequest(); setButtonLoading('btnDisplayStatus', true, 'Lade Status'); fetchJsonNoStore('/dev/display-status').then((r) => r.json()).then((data) => { updateText('displayReadyValue', data.ready ? 'Bereit' : 'Nicht bereit'); updateText('displayTouchValue', data.touchReady ? 'Touch aktiv' : 'Touch fehlt'); updateText('displayBuffersValue', data.buffersAllocated ? 'DMA ok' : 'Keine Buffer'); updateText('displayTickValue', data.tickFallback ? 'Loop' : 'Timer'); updateText('displayErrorValue', data.lastError || 'Kein Fehler'); setPill('displayStatusPill', data.ready ? 'ok' : (data.lastError ? 'bad' : 'warn'), data.ready ? 'Display bereit' : 'Display pruefen'); }).catch(() => { updateText('displayErrorValue', 'Request fehlgeschlagen'); setPill('displayStatusPill', 'bad', 'Display Fehler'); }).finally(() => { setButtonLoading('btnDisplayStatus', false); done(); }); }
function probeAmbientSensor(){ const done = beginRequest(); setButtonLoading('btnAmbientProbe', true, 'Pruefe Sensor'); fetch('/dev/ambient-probe', { method:'POST', cache:'no-store' }).then((r) => r.json()).then(() => fetchStatus()).catch(() => {}).finally(() => { setButtonLoading('btnAmbientProbe', false); done(); }); }
function probeGestureSensor(){ const done = beginRequest(); setButtonLoading('btnGestureProbe', true, 'Pruefe Sensor'); fetch('/dev/gesture-probe', { method:'POST', cache:'no-store' }).then((r) => r.json()).then(() => fetchStatus()).catch(() => {}).finally(() => { setButtonLoading('btnGestureProbe', false); done(); }); }
...
function maybeAutoProbeSensors(data){ const now = Date.now(); if(!ambientSensorOnline(data) && (now - dashboardState.lastAmbientAutoProbeMs) > 6000){ dashboardState.lastAmbientAutoProbeMs = now; fetch('/dev/ambient-probe', { method:'POST', cache:'no-store' }).then(() => fetchStatus()).catch(() => {}); } if(data?.gestureControlEnabled && !gestureSensorOnline(data) && (now - dashboardState.lastGestureAutoProbeMs) > 6000){ dashboardState.lastGestureAutoProbeMs = now; fetch('/dev/gesture-probe', { method:'POST', cache:'no-store' }).then(() => fetchStatus()).catch(() => {}); } }
function updateStatus(data){
  if(!data){ return; }
  const modeChanged = syncSelectValue('modeSelect', data.mode, true);
  ...
  if(modeChanged){ updatePreviewLoop(); }
  maybeAutoProbeSensors(data);
...
function fetchStatus(){ const done = beginRequest(); fetchJsonNoStore('/status').then((r) => r.json()).then(updateStatus).catch(() => {}).finally(done); }
function saveForm(url){ const form = $('#mainForm'); if(!form){ return Promise.resolve(); } const done = beginRequest(); setButtonLoading('btnSave', true, 'Speichere'); return fetch(url, { method:'POST', cache:'no-store', body:new FormData(form) }).then(() => { captureInitialState(); recomputeDirty(); return fetchStatus(); }).finally(() => { setButtonLoading('btnSave', false); done(); }); }
...
function saveSettings(){ const form = $('#settingsForm'); if(!form){ return Promise.resolve(); } const fd = new FormData(form); const params = new URLSearchParams(); fd.forEach((value, key) => params.append(key, value)); const done = beginRequest(); setButtonLoading('settingsSave', true, 'Speichere'); return fetch('/settings', { method:'POST', cache:'no-store', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:params.toString() }).then(() => { captureInitialSettingsState(); recomputeSettingsDirty(); $('#savedToast')?.classList.remove('hidden'); fetchStatus(); fetchWifiStatus(); fetchBleStatus(); refreshAdvancedDisplay(); }).finally(() => { setButtonLoading('settingsSave', false); done(); }); }
...
function fetchStatus(){ fetchJsonNoStore('/status').then((r) => r.json()).then(updateStatus).catch(() => {}); }
...
function fetchWifiStatus(){ fetchJsonNoStore('/wifi/status').then((r) => r.json()).then(updateWifiUi).catch(() => {}); }
...
function fetchBleStatus(){ fetchJsonNoStore('/ble/status').then((r) => r.json()).then(updateBleUi).catch(() => {}); }
...
function refreshAdvancedDisplay(){ const done = beginRequest(); fetchJsonNoStore('/dev/display-status').then((r) => r.json()).then((data) => { updateText('advDisplayReady', data.ready ? 'Bereit' : 'Nicht bereit'); updateText('advDisplayTouch', data.touchReady ? 'Touch aktiv' : 'Touch fehlt'); updateText('advDisplayLastError', data.lastError || 'Kein Fehler'); }).finally(done); }
